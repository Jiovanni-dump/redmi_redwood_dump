...
function dump() {
    ...
    rm -rf ${source_traces_path}/*performance.perfetto-trace
    rm -rf ${dest_traces_path}/*performance.perfetto-trace
    cd ${source_traces_path}

    while true
        do
            datetime=$(date +"%Y%m%d-%H%M%S")
            dest_file="trace-$board-$id-$datetime-performance.perfetto-trace"
